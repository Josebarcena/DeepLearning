...
inputs106 = Float32.(getInstancia(posicionesInst1,inputs106,20));
...
inputs208 = Float32.(getInstancia(posicionesInst2,inputs208,20));





train_inputs = inputs208;
train_labels = booleanosInst2;
test_inputs = inputs106;
test_labels = booleanosInst1;
labels = 1:5;

#train_imgs   = load("../../aa/Funciones/MNIST.jld2", "train_imgs");
#train_labels = load("../../aa/Funciones/MNIST.jld2", "train_labels");
#test_imgs    = load("../../aa/Funciones/MNIST.jld2", "test_imgs");
#test_labels  = load("../../aa/Funciones/MNIST.jld2", "test_labels");
#labels = 0:9;
...
println("Tamaño de la matriz de entrenamiento: ", size(train_inputs))
println("Tamaño de la matriz de test:          ", size(test_inputs))


# Cuidado: en esta base de datos las imagenes ya estan con valores entre 0 y 1
# En otro caso, habria que normalizarlas
println("Valores minimo y maximo de las entradas: (", minimum(train_inputs), ", ", maximum(train_inputs), ")");
...
gruposIndicesBatch = Iterators.partition(1:size(train_inputs,1), batch_size);
...
train_set = [ (train_inputs[:,:,:,indicesBatch], onehotbatch(train_labels[indicesBatch], labels)) for indicesBatch in gruposIndicesBatch];

# Creamos un batch similar, pero con todas las imagenes de test
test_set = (test_inputs, onehotbatch(test_labels, labels));
...
numBatchCoger = 1; numImagenEnEseBatch = [10];
